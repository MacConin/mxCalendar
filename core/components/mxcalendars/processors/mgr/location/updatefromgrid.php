...
//-- Check for the required location ID
if (empty($_DATA['id'])) 
	return $modx->error->failure($modx->lexicon('mxcalendars.err_ns'));

//-- Now check to make sure that the location item exist and can be updated
$mxcalendar = $modx->getObject('mxCalendarLocations',$_DATA['id']);
if (empty($mxcalendar)) 
